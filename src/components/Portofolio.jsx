...
function ProjectCard({ item, index }) {
  return (
    <article className="group flex gap-4 border-2 border-[#202321] bg-[#f7f6f0] p-3 shadow-[3px_3px_0_#202321] transition hover:-translate-y-1">
      <a href={item.link} target="_blank" rel="noreferrer" aria-label={`Open ${item.title}`} className="relative h-24 w-24 shrink-0 overflow-hidden border-2 border-[#202321] bg-[#dfe1da] sm:h-28 sm:w-28"><img src={item.image} alt={item.title} loading="lazy" className="h-full w-full object-cover grayscale-50 transition duration-500 group-hover:scale-105 group-hover:grayscale-0" /><span className="absolute bottom-0 left-0 border-r-2 border-t-2 border-[#202321] bg-[#68bcb9] px-1.5 mono text-[9px] font-bold">0{index + 1}</span></a>
